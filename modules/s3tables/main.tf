...
locals {
  ...
  # Existing S3 Tables buckets map (you already have this)
  table_bucket_names = {
  ...
  # NEW: standard S3 buckets map -> {index => name}
  lake_bucket_names = {
    for i in range(var.start_index, var.start_index + var.bucket_count) :
...
# Strongly recommended controls for new buckets
...
resource "aws_s3_bucket_server_side_encryption_configuration" "lake" {
  ...
  rule {
    apply_server_side_encryption_by_default {
      sse_algorithm = "AES256" # swap to aws:kms + kms_master_key_id if you prefer KMS
    }
  }
}

# Optional bucket policy granting your principals access
resource "aws_s3_bucket_policy" "lake" {
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      # Object-level access
      {
        Sid:    "AllowObjectRWForPrincipals",
        Effect: "Allow",
        Principal: { AWS: var.policy_principals },
        Action: [
          "s3:GetObject","s3:PutObject","s3:DeleteObject",
          "s3:GetObjectVersion","s3:AbortMultipartUpload",
...
# ---------- S3 TABLES (you already had these) ----------
